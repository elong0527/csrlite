...
def test_apply_common_filters_no_filters():
    ...
    assert res_pop.equals(pop)
    assert res_obs.equals(obs)

def test_apply_common_filters_population_filter():
    pop = pl.DataFrame({"id": [1, 2, 3], "group": ["A", "B", "A"]})
    obs = pl.DataFrame({"id": [1, 2, 3], "val": [10, 20, 30]})
...
def test_apply_common_filters_observation_filter():
    ...
    assert res_pop.equals(pop)
    assert res_obs.equals(expected_obs)

def test_apply_common_filters_parameter_filter():
    pop = pl.DataFrame({"id": [1, 2, 3]})
...
def test_apply_common_filters_all_filters():
    pop = pl.DataFrame({"id": [1, 2, 3], "group": ["A", "B", "A"]})
    obs = pl.DataFrame({"id": [1, 2, 3], "val": [10, 20, 30], "param": ["X", "Y", "X"]})
    
    res_pop, res_obs = apply_common_filters(
        pop, 
        obs, 
        "group == 'A'", 
        "val > 0", 
